        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          ...
          <div>
            ...
            <ul className="space-y-4">
              {['Home', 'About Us', 'Our Work', 'On Tour', 'Traditions'].map((item) => (
                <li key={item}>
                  <Link to={`/${item === 'Home' ? '' : item === 'Our Work' ? 'programs' : item.toLowerCase().replace(' ', '')}`} className="text-sm text-white/50 hover:text-bai-red transition-colors">
                    {item}
                  </Link>
                </li>
              ))}
